...
#include <ament_index_cpp/get_package_prefix.hpp>  // 추가됨
#include <rosidl_typesupport_introspection_cpp/message_introspection.hpp>
#include <rosidl_typesupport_introspection_cpp/field_types.hpp>
#include <rosbag2_cpp/typesupport_helpers.hpp>
#include <rosbag2_cpp/types/introspection_message.hpp>
#include <rcutils/time.h>
#include <rcpputils/shared_library.hpp>  // 추가됨
#include <functional>
#include <cmath>
  ...
  const std::string &type_name)
{
  ...
  //----- 패키지 이름 추출 -----
  std::string package_name = type_name.substr(0, type_name.find('/'));
  std::string prefix = ament_index_cpp::get_package_prefix(package_name);
  std::string library_path = prefix + "/lib/lib" + package_name + "__rosidl_typesupport_introspection_cpp.so";

  //----- 라이브러리 로드 -----
  auto type_library = std::make_shared<rcpputils::SharedLibrary>(library_path);

  info.type_support =
    rosbag2_cpp::get_typesupport_handle(type_name, rosidl_typesupport_introspection_cpp::typesupport_identifier, type_library);
